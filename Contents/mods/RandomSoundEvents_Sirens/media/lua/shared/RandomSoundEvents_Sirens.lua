...
local function canPlay(player, x, y)
    ...
    return not SandboxVars.RandomSoundEvents_Sirens.disabled and worldAge >= SandboxVars.RandomSoundEvents_Sirens.daysSinceApocalypse and worldAge < SandboxVars.RandomSoundEvents_Sirens.daysSinceApocalypseEnd;
end
    ...
    for i = 0, 3 do
        ...
        if player and not player:isDead() and not player:isGodMod() then
            if Utils.IsInRange(soundRange, x, y, player:getX(), player:getY()) then
                ...
                if not SandboxVars.RandomSoundEvents_Sirens.disableFear then
